...
class GetNewsListSearchUseCase @Inject constructor(
    private val newsRepository: NewsRepository
) {
    operator fun invoke(query:String, token: String, country: String,page: Int): Flow<Resource<List<Article>>> {
        return flow {
            emit(Resource.Loading) // emit loading state

            // call the getTopHeadlines API method
            val newsResponse = newsRepository.getNewsSearch(token,country,page,query)
